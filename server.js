...
let database;
const databaseUrl = `mongodb://localhost:27017/eatup`;
...
MongoClient.connect(databaseUrl, {useNewUrlParser: true}, function (err, client) {
  ...
  var location_collection = database.collection('location_search');
});
...
api.get('/shops', (request, response) => {
...
api.get('/api/shops',(request,response)=> {
  ...
  database.collection('location-search').find().toArray((err,result)=> {
    if (err) throw err;
    response.send(result);
  })
});
*/
api.get('/shops/topten', (request, response) => {
...
api.get('/shops/newest', (request, response) => {
...
api.get('/shops/:shopId', (request, response) => {
  const query = {
...
api.get('/shops/:shopId/consumables', (request, response) => {
  const query = {
...
api.post('/shops/:shopId/food', bodyParser.json(), (request, response) => {
...
api.post('/shops/:shopId/beverages', bodyParser.json(), (request, response) => {
  console.log((request.body));
  response.send()
});

// api.get('/api/shops',(request,response)=> {
// //   //gives back the locations stored in db
//     database.collection('location-search').find().toArray((err,result)=> {
//       if (err) throw err;
//       response.send(result);
//     })
